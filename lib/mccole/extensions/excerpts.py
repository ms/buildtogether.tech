...
-   `[% excerpt file="path" %]` includes an entire file. The path is
    relative to the including file.

-   `[% excerpt file="path" keep="key" %]` includes everything between
    lines marked with `[key]` and `[/key]`.  (These markers are usually
    placed in comments.)

-   `[% excerpt file="path" omit="key" %]` omits everything between
    markers.

-   `[% excerpt file="path" keep="outer" omit="inner" %]` selects the
...
def _file(inclusions, node, file):
    ...
    return _include_file(node, filepath, file)
...
def _keep(inclusions, node, file, keep):
    ...
    return _include_file(
        node, filepath, file, lambda lns: _keep_lines(filepath, lns, keep)
    )
...
def _omit(inclusions, node, file, omit):
    ...
    filepath = _inclusion_filepath(inclusions, node, file)
    return _include_file(
        node, filepath, file, lambda lns: _omit_lines(filepath, lns, omit)
    )
...
def _include_file(node, filepath, file, *filters):
    ...
    try:
        with open(filepath, "r") as reader:
            ...
            for f in filters:
                lines = f(lines)
            return _make_html(filepath, file, kind, lines)
    except OSError:
...
def _make_html(filepath, file, kind, lines):
